...
> - **Still open (Slices C/D):** AVAssetWriter take sink + take events, body/face Metal passes consuming the matte, model packs (`pms_model_status` fake), Metal numeric tests (`tools/metal_render_test.mm` has no build target), text baked into the engine render path, CI workflow file, on-device §6 scenarios.
