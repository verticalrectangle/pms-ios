...
> - **Still open:** engine-side loop/take events on iOS, face beauty/warp/makeup passes (need face-observation plumbing to the engine on iOS), numeric Metal render tests (no build target yet), model-pack download flow, on-device §6 scenarios.
