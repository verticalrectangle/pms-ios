...
> - **Still open (Slices C/D):** mic injection (`pms_submit_mic_block` is a header stub; no `audio_capture_push` ring yet), Vision matte ABI + matte/body/face Metal passes, take sink + take events wiring, model packs (`pms_model_status` fake), Metal numeric tests, text baked into the engine render path (export uses the shared CI compositor; canvas preview overlay remains Swift), CI workflow file, ABI bump to 2 when the matte call lands.
